...
import org.github.ewt45.winemulator.Utils.Files.writeStringToFileWithLF
...
object ProotHelper {
    ...
    fun setup_fake_data() {
        ...
        writeIfNotExists("/proc/.loadavg", "0.12 0.07 0.02 2/165 765")
        writeIfNotExists("/proc/.stat", """
            ...
            """.trimIndent())
        writeIfNotExists("/proc/.uptime", "124.08 932.80")
        writeIfNotExists("/proc/.version", "Linux version ${DEFAULT_FAKE_KERNEL_VERSION} (proot@termux) (gcc (GCC) 12.2.1 20230201, GNU ld (GNU Binutils) 2.40) #1 SMP PREEMPT_DYNAMIC Wed, 01 Mar 2023 00:00:00 +0000")
        writeIfNotExists("/proc/.vmstat", """
            ...
            nr_unstable 0
            """.trimIndent())
        writeIfNotExists("/proc/.sysctl_entry_cap_last_cap", "40")
...
//        }
    ...
    private fun writeIfNotExists(filePath: String, str: String) {
        File(rootfsCurrDir, filePath).takeIf { it.notExists() }?.let {
            FileUtils.writeStringToFile(it, str, StandardCharsets.UTF_8)
